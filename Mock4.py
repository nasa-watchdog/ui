...
class TaskWindow:
	...
	def __init__(self, num, c):
		...
		if self.case == 3:
			...
			cv2.bind('<Button-1>',self.ShowWarn)
	...
	def OnNext(self):
		...
		if self.n == 6:
			...
			if (Case_save[self.n] == 1) and (Case_save[self.n+1] == 2):
				...
				self.root.destroy()
				win.root.after(3000,win.Signal_Update)
			else:
	...
	def OnBack(self):
		...
		if self.n == 0:
			...
			tkinter.messagebox.showwarning(title, "Warning! It is the first Task.",parent = self.root)
	...
	def ShowWarn(self,event):
		...
		tkinter.messagebox.showwarning(titLE, "1. Use the bluetooth and body camera to search the samples;\n2. Use the body camera to identify those target samples;\n3. Use the hammer to collect some samples.",parent = self.root)
